<template>
  <div class="btn-group float-right" name="HeaderSettings">
    <button class="btn btn-secondary mr-1" ref="printBtn" type="button" @click="printPage">
      <fa icon="print" fixed-width />
    </button>
    <button data-toggle="dropdown" class="btn btn-secondary dropdown-toggle" ref="dropdownBtn" type="button">
      <fa icon="cog" fixed-width />
      <fa icon="caret-down" fixed-width />
    </button>
    <div class="dropdown-menu dropdown-menu-right keep-open clearfix" :style="drpMenuStyle">
      ...
      <div class="clearfix" style="margin: 10px 0">
        <div class="btn-group btn-group-sm pull-right">
          <button class="btn btn-secondary closes-dropdown" type="button" @click="apply()">
            {{ $i18nForDatatable('Apply') }}
          </button>
          <template v-if="supportBackup">
            <button data-toggle="dropdown" class="btn btn-secondary dropdown-toggle" type="button" style="box-shadow: none">
...
export default {
  ...
  methods: {
    ...
    printPage(){
      // let html = document.querySelector('.page-content').innerHTML
      // html = '<html><head>' + document.querySelector('head').innerHTML + '</head><body>' +  + '</body></html>'
